...
static int8_t user_spi_read(uint8_t dev_id, uint8_t reg_addr, uint8_t *reg_data,
                            uint16_t len) {
  /*
   * The parameter dev_id can be used as a variable to select which Chip Select
   * pin has to be set low to activate the relevant device on the SPI bus
   */
...
static int8_t mgos_bme280_get_sensor_data(uint8_t sensor_comp,
...
int8_t mgos_bme280_read(struct mgos_bme280 *bme,
                        struct mgos_bme280_data *data) {
  if (NULL == bme) {
    return -1;
  }
  /* Don't try to read humidity if BMP280 */
  uint8_t sensor_comp = (BME280_CHIP_ID == bme->dev.chip_id)
                            ? BME280_ALL
                            : (BME280_PRESS | BME280_TEMP);
  struct bme280_data comp_data;
...
/*
 */
bool mgos_bme280_getStats(struct mgos_bme280 *bme,
                          struct mgos_bme280_stats *stats) {
  if (!bme || !stats) return false;

  memcpy((void *) stats, (const void *) &bme->stats,
         sizeof(struct mgos_bme280_stats));
  return true;
}
